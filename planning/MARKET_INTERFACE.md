...
class PriceUpdate:
    ...
    change_pct: float         # (price - previous_price) / previous_price * 100
...
Shared in-memory store that the data source writes to and the SSE streamer reads from.
...
The SSE endpoint reads from `PriceCache` and pushes one event per ticker per 500ms tick:
...
async def price_stream(price_cache: PriceCache):
    """SSE generator that yields one event per ticker every 500ms."""
    yield "retry: 3000\n\n"
        ...
        for ticker, p in prices.items():
            payload = {
                ...
                "data": {
                    ...
                    "timestamp": datetime.utcfromtimestamp(p.timestamp).isoformat() + "Z",
                },
            }
...
2. **Watchlist add**: Call `await source.add_ticker(ticker)` — takes effect on next poll/step
3. **Watchlist remove**: Call `await source.remove_ticker(ticker)` — removes from cache immediately
4. **SSE streaming**: Reads from `price_cache.get_all()` every 500ms, emits one event per ticker
5. **Trade execution**: Reads current price via `price_cache.get(ticker).price`
6. **App shutdown**: Call `await source.stop()`
